...
app.get(["/", "/index", "/home"], (req, res) => {
    client.query("select * from articol", (err, prods) => {
        if(err) console.log(err);
        else res.render("pagini/index", {ip: req.ip, imagini: obGlobal.obImagini.imagini, prods: prods.rows});
    });
});
...
app.get("/produse", function(req,res) {
    ...
    client.query("select * from unnest(enum_range(null::grupa_de_varsta))", function(err,varste) {
        client.query("select * from unnest(enum_range(null::categ_articol))", (err, tip) => {
            client.query(`select * from articol ${conditieQuery}`, function(err,rez){
                if(err){
                    console.log(err);
                    afisareEroare(res,2);
                } else {
                    res.render("pagini/produse", {produse: rez.rows, varste:varste.rows, tip:tip.rows});
                }
            })
        })
...
app.get("/produs/:id", function(req,res) {
    client.query(`select * from articol where id=${req.params.id}`, function(err,rez){
        if(err){
            console.log(err);
            afisareEroare(res,2);
        } else {
...
app.post("/inregistrare",function(req, res){
    ...
    formular.parse(req, function(err, campuriText, campuriFisier ){//4
        ...
        try{
            ...
            utilizNou.nume=campuriText.nume[0];
    ...
    */
    ...
    formular.parse(req, function(err, campuriText, campuriFisier ){
        ...
        Utilizator.getUtilizDupaUsername (campuriText.username[0],parametriCallback, function(u, obparam ){
            ...
            if(u.parola == parola && u.confirmat_mail ){
...
function compileazaScss(caleScss, caleCss){
    ...
    if (fs.existsSync(caleCss)){
        fs.copyFileSync(caleCss, path.join(obGlobal.folderBackup, "resurse/css",numeFisCss.split(".")[0] + new Date().getTime() + ".css"))// +(new Date()).getTime()
    }
